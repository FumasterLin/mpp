...
MPP_RET hal_h264e_vepu1_deinit(void *hal)
{
    ...
    if (ret)
        mpp_err("mpp_dev_deinit failed ret: %d", ret);
...
MPP_RET hal_h264e_vepu1_start(void *hal, HalTaskInfo *task)
{
    ...
    if (ctx->dev_ctx) {
        ...
        do {
            ...
            RK_U32 reg_size = VEPU_H264E_VEPU1_NUM_REGS * sizeof(RK_U32);//sizeof(ctx->regs_set);
...
static MPP_RET hal_h264e_vepu1_set_feedback(h264e_feedback *fb, void *reg)
{
    RK_S32 i = 0;
...
static MPP_RET hal_h264e_vpu1_resend(void *hal, RK_U32 *reg_out, RK_S32 dealt_qp)
{
    H264eHalContext *ctx = (H264eHalContext *)hal;
    ...
    hw_ret = mpp_dev_ioctl(ctx->dev_ctx, MPP_DEV_CMD_POLL, NULL);//mpp_device_wait_reg(ctx->dev_ctx, (RK_U32 *)reg_out, VEPU_H264E_VEPU1_NUM_REGS);
    if (hw_ret) {
...
MPP_RET hal_h264e_vepu1_wait(void *hal, HalTaskInfo *task)
{
    ...
    if (ctx->dev_ctx) {
        RK_S32 hw_ret = mpp_dev_ioctl(ctx->dev_ctx, MPP_DEV_CMD_POLL, NULL);
